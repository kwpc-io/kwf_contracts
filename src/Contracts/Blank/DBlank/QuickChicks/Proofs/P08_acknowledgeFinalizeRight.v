...
Lemma acknowledgeFinalizeRight_remove_conditions_helper :
...
Proof.
  ...
  apply (acknowledgeFinalizeRight_remove_conditions_helper _ _ _ _ _ Hnoreq).
...
Lemma acknowledgeFinalizeRight_noexec_proof : acknowledgeFinalizeRight_noexec_prop giver nonce dead_giver l.
Proof.
  unfold acknowledgeFinalizeRight_noexec_prop. remember (exec_state _ _) as l'.
  rewrite <- acknowledgeFinalizeRight_exec_prf in Heql'.
  unfold acknowledgeFinalizeRight_exec in Heql'.
  intros Hnoreq. remember acknowledgeFinalizeRight_isError_proof as prf. clear Heqprf.
  unfold acknowledgeFinalizeRight_isError_prop in prf. rewrite isErrorEq in prf.
  rewrite <- prf in Hnoreq. clear prf.
  rewrite <- acknowledgeFinalizeRight_err_prf in Hnoreq.
  unfold acknowledgeFinalizeRight_err in Hnoreq.
  destruct l eqn:eql. rewrite <- eql in *. repeat destruct p.
  destruct c. repeat destruct p.
  destruct v. repeat destruct p.
